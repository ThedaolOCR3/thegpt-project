...
관리자 페이지에서 문서 OCR, 청크 설정, Jina v4·Medical BGE-M3 이중 Embedding 기반 VectorDB 저장, LLM 응답 비교 기능을 제공합니다. 메인 페이지에서는 두 Dense 검색 순위를 RRF로 합친 의료 RAG와 공통 Model Registry의 LLM을 사용합니다. PDF·이미지·DOCX/PPTX·JSON·JSONL·CSV·TXT와 이 형식을 담은 ZIP을 처리합니다.
