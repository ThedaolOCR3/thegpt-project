"""설정값(`Settings.rag_embedding_*`)으로 `ai.rag`의 `EmbeddingProvider`를 만든다.
...
않아도 된다. (`embedding_*` 설정은 OCR→Gemini 임베딩 파이프라인
(`embedding_service.py`, `document_chunks.embedding` 컬럼)용으로 완전히 별개다 —
여기서 건드리지 않는다.)
"""
from functools import lru_cache

from ai.rag import EmbeddingProvider, HashingEmbeddingProvider
...
def get_default_rag_embedding_providers() -> list[EmbeddingProvider]:
    ...
    return [build_rag_embedding_provider(settings)]
