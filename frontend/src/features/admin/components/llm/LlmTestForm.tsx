  ...
  return (
    <div className="admin-card llm-form-card">
      ...
      <div className="llm-attachment-row">
        ...
        <small>
          파일명만 요청 상태에 포함되며 파일은 읽거나 업로드하지 않습니다.
        </small>
      </div>
      <fieldset className="model-fieldset" disabled={loading}>
        <legend>
          비교 모델 <span>2개 이상 선택</span>
        </legend>
        <div className="model-choice-grid">
          {MODEL_OPTIONS.map((model) => (
            <label
              className={`model-choice ${props.modelIds.includes(model.id) ? "selected" : ""}`}
              key={model.id}
            >
              <input
                type="checkbox"
                checked={props.modelIds.includes(model.id)}
                onChange={() => props.onToggleModel(model.id)}
              />
              <span>{model.label}</span>
              <small>
                {model.id === "llama" ? "부분 오류 fixture" : model.tier}
              </small>
            </label>
          ))}
        </div>
      ...
      <div className="llm-options">
        <label className="admin-field">
          ...
          <input
            ...
            disabled={loading}
            onChange={(event) =>
              props.onChunkSizeChange(Number(event.target.value))
            }
          />
        </label>
        <label className="admin-field">
          ...
          <input
            ...
            disabled={loading}
            onChange={(event) =>
              props.onOverlapChange(Number(event.target.value))
            }
          />
        </label>
      </div>
      ...
      <div className="admin-form-actions">
        ...
        >
          {loading ? (
            <>
              <LoaderCircle className="spin" size={17} /> 비교 생성 중...
            </>
          ) : (
            "모델 응답 비교"
          )}
