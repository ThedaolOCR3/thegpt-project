...
export function createOcrMockResult(documentName: string, isPdf: boolean): OcrDocumentResult {
  return {
    documentName,
    pageCount: isPdf ? 4 : 1,
    ...
    extractedText: '환자의 현재 증상과 과거 병력을 함께 검토해야 합니다. 문서에 포함된 검사 결과는 임상적 판단을 보조하기 위한 참고 자료이며, 최종 진단은 의료 전문가의 확인이 필요합니다. 복용 중인 약물과 알레르기 정보를 먼저 확인하고 필요한 추가 검사를 결정합니다.',
    chunks: [
