...
export function useLlmComparison() {
  ...
  async function compare() {
    ...
    setError(''); setResults([]); setStatus('loading');
    try {
      setResults(await adminAiService.compareModels({ prompt: prompt.trim(), modelIds, file: file ?? undefined, chunkSize, overlap }));
      setStatus('success');
    } catch (unknownError) {
      setError(unknownError instanceof Error ? unknownError.message : '모델 비교 테스트에 실패했습니다.');
      setStatus('error');
    }
  }

  function reset() {
    setPrompt(''); setModelIds(DEFAULT_LLM_MODEL_IDS); setFile(null); setChunkSize(512); setOverlap(50);
    setResults([]); setError(''); setStatus('idle');
  }

  return { prompt, setPrompt, modelIds, toggleModel, file, setFile, chunkSize, setChunkSize, overlap, setOverlap, status, results, error, compare, reset };
}
